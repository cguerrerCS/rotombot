...
function classifyRaiders(raiders) {
    ...
    classified.details = "";
    if (numPending > 0) {
        classified.summary = ((classified.inbound.length < 1) ? `(${classified.present.length})` : `(${classified.present.length}/${numPending})`);
    }

    classified.inbound.forEach((r) => { classified.details += `    ${r.toString()}\n`; });
    classified.present.forEach((r) => { classified.details += `    ${r.toString()}\n`; });
    classified.raiding.forEach((r) => { classified.details += `    ${r.toString()}\n`; });
    return classified;
}
