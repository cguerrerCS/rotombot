...
const raidCommands = [
    ...
    },
    {
        name: "EX raids",
        description: [
            "Raids at EX-eligible gyms:",
            "  !raids ex [in <cities>]",
            "  !raids ex in rose hill",
        ],
        pattern: /^!raids\s+ex\s*(?:\s+in\s+(\w(?:\s|\w|,)*))?\s*$/i,
        handleCommand: (matches) => {
            return { minTier: 1, maxTier: 5, place: matches[1], exOnly: true };
        },
    },
    {
        name: "MEGA raids",
        description: [
            "Mega raids:",
            "  !raids mega [in <cities>]",
            "  !raids m in rose hill",
        ],
        pattern: /^!raids\s+(?:mega|m)\s*(?:\s+in\s+(\w(?:\s|\w|,)*))?\s*$/i,
        handleCommand: (matches) => {
            return { minTier: 4, maxTier: 4, place: matches[1], exOnly: false };
        },
    },
    {
...
function tryParse(command, rm, allowEmbeds) {
    ...
    let showRsvp = false;
    Utils.extract(command, "+rsvp", (c) => {
        showRsvp = allowEmbeds;
        command = c;
    });
    ...
    want.filter = function (r) {
        ...
        if (match) {
            if (want.exOnly) {
                match = (match && r.gym.isExEligible) ? true : false;
            }
    ...
    let tierDescription = ((want.minTier === want.maxTier) ? `T${want.minTier}` : `T${want.minTier}-T${want.maxTier}`);
    if (tierDescription === "T4") {
        tierDescription = "MEGA";
    }

    want.filterDescription = (tierDescription + (want.place ? ` IN ${want.place.toUpperCase()}` : ""));
    want.showListCard = showListCard;
    want.showRaidCards = showRaidCards;
    want.showRsvp = showRsvp;
    return want;
}
...
function getMessagesForCommand(want, rm, lookupOptions) {
    ...
    if (want.showListCard) {
        let cardHeadings = {
            description: want.filterDescription,
        };

        let extras = rm.hooks.getMessages("raids");
        if (extras && (extras.length > 0)) {
            cardHeadings.extraText = extras.join("\n");
        }

        let msg = Raid.raidListToDiscordMessage(raids, cardHeadings);
    ...
    if (want.showRsvp) {
        let message = Raid.raidListToRaiderSummaryCard(raids);
        if (message) {
