...
var UserConfig = function (init, id) {
    id = id || init.id;
    if ((!id) || (typeof id !== "string") || (id.trim().length < 1)) {
        throw new Error(`User ID "${id}" must be a non-empty string.`);
    }
...
UserConfigManager.prototype.addOrUpdateUser = function (userInit, id) {
    id = id || userInit.id;
    this.users[id] = new UserConfig(userInit, id);
    this.dirty = true;
