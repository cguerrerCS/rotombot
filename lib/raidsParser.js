...
module.exports = {
    tryParse: function (command) {
        let baseCommand = command.replace(/\+directions/gi, "");
        let showDirections = (command !== baseCommand);
        command = baseCommand;
        baseCommand = command.replace(/\+rsvp/gi, "");
        let showRsvps = (command !== baseCommand);

        let cmdResult = raidProcessor.processFirst(baseCommand);
        if (!cmdResult.found) {
            return undefined;
        }
        ...
        want.showDirections = showDirections;
        want.showRsvps = showRsvps;
        return want;
    },
};
