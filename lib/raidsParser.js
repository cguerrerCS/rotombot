...
RaidsParser.tryParse = function (command, blockEmbeds) {
    let allowEmbeds = (blockEmbeds ? false : true);
    ...
    if (want.city && want.city.trim().match(/^all$/i)) {
        want.allZones = true;
        want.city = "ALL ZONES";
    }

    if ((!want.allZones) && want.city && want.city.length > 0) {
...
RaidsParser.getMessagesForCommand = function (want, rm, lookupOptions) {
    ...
    if ((!want.haveCity) && (!want.allZones)) {
