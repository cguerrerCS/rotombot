...
Utils.formatTime24Hours = function (date) {
...
Utils.dateToUnixTimestamp = function (date) {
...
Utils.validateDiscordEmbed = function (msg) {
    let titleLength = msg.title ? msg.title.length : 0;
    let descriptionLength = msg.description ? msg.description.length : 0;
    let footerLength = msg.footer && msg.footer.text ? msg.footer.text.length : 0;
    let authorLength = msg.author && msg.author.name ? msg.author.name.length : 0;
    let totalLength = titleLength + descriptionLength + authorLength + footerLength;

    let ok = (titleLength < 256) && (descriptionLength < 2048) && (authorLength < 256) && (footerLength < 2048);

    if (ok && msg.fields && (msg.fields.length > 0)) {
        ok = ok && (msg.fields.length <= 25);
        for (let i = 0; ok && (i < msg.fields.length); i++) {
            let nameLength = msg.fields[i].name.length;
            let valueLength = msg.fields[i].value.length;
            totalLength += nameLength + valueLength;
            ok = ok && (nameLength < 256) && (valueLength < 1024);
        }
    }

    ok = ok && (totalLength < 6000);
    return ok;
};
