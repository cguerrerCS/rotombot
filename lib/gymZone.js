...
function adjustForCities(candidates, options) {
    let matched = [];
    let unmatched = [];
...
GymZone.prototype.tryGetGyms = function (name, options) {
    ...
    if (exact && ((!options) || (!options.noExact))) {
    ...
    else if ((!options) || (!options.noFuzzy)) {
    ...
    if ((candidates.length > 1) && (options && options.cities && (options.cities.length > 0))) {
        candidates = adjustForCities(candidates, options);
    }
