...
RaidTrain.suggestStartTimeForRaid = function (proposed, raid) {
    if (proposed.getTime() < raid.hatchTime.getTime()) {
        return {
            time: raid.hatchTime,
            warning: `${Utils.formatTimeAmPm(proposed)} is earlier than hatch time ${Utils.formatTimeAmPm(raid.hatchTime)}`,
        };
    }
    else if (proposed.getTime() > Utils.getDeltaDate(raid.expiryTime, -3).getTime()) {
        let proposedStr = Utils.formatTimeAmPm(proposed);
        let expiryStr = Utils.formatTimeAmPm(raid.expiryTime);
        let reason = `${proposedStr} is too close to raid expiration time ${expiryStr}`;
        if (proposed.getTime() > raid.expiryTime.getTime()) {
            reason = `${proposedStr} is after raid expiration time ${expiryStr}`;
        }

        return {
            time: Utils.getDeltaDate(raid.expiryTime, -3),
            warning: reason,
        };
    }
    return {
    ...
    if (this.stops.length > 0) {
        ...
        for (let i = 1; i < this.stops.length; i++) {
            if (this.stops[i].scheduledStart) {
                ...
                let estimated = Utils.getDeltaDate(lastRaidTime, 15);
                let suggested = RaidTrain.suggestStartTimeForRaid(estimated, this.stops[i].raid);
                this.stops[i].estimatedStart = suggested.time;
                if (suggested.warning) {
                    this.stops[i].warning = suggested.warning;
                }
            }
