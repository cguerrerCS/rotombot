...
RaidManager.prototype.setGymData = function (gyms) {
    if (!(gyms instanceof GymDirectory)) {
        throw new Error(`Cannot intialize gym data: expecte GymDirectory, got ${typeof gyms}.`);
    }
...
RaidManager.prototype.validateGym = function (wantGym) {
    ...
    var possibleGyms = this.gyms.tryGetGyms(wantGym);
...
RaidManager.prototype.tryGetRaid = function (gymName) {
    let normalized = Gym.normalizeName(gymName);
...
function raidToString(raidObj) {
    ...
    return `[Location: ${raidObj.gym.friendlyName}, Now: ${now}, SpawnTime: ${spawn}, HatchTime: ${hatch}, ExpiryTime: ${expiry}]`;
}
...
RaidManager.prototype.restoreFromSaveState = function (allSaved) {
    ...
    allSaved.forEach((saved) => {
        let now = Date.now();
        if (saved.expiry > now) {
            let gyms = self.gyms.tryGetGymsExact(saved.gym);
            if (gyms !== undefined) {
                if (gyms.length === 1) {
                    let boss = (saved.boss ? self.bosses.find((b) => b.name === saved.boss) : undefined);
                    let raid = self._forceRaid(gyms[0], saved.tier, boss, new Date(saved.hatch));
