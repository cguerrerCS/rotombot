...
const defaultOptions = {
    strict: true,
    logger: null,
    autosaveFile: "./state/latest.json",
    refresh: 30,
};
...
var RaidManager = function (options) {
    ...
    this.autosaveFile = (options.autosaveFile ? path.resolve(path.normalize(options.autosaveFile)) : undefined);
...
RaidManager.prototype.list = function (filter) {
    ...
    Utils.forOwn(this.raids, (raid) => {
        ...
        if (filter(raid)) {
            var raidCopy = Object.assign({}, raid);
            sortedRaids.push(raidCopy);
        }
    });
