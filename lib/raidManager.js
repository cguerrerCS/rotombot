...
RaidManager.prototype.addEggCountdown = function (wantTier, wantGym, wantMinutes) {
    ...
    if (this.strict && (raid.raidLocation.name in this.raids)) {
        throw new Error(`Raid already reported for ${raid.raidLocation.name}.`);
    }

    this.raids[raid.raidLocation.name] = raid;
    return raid;
};
...
RaidManager.prototype.listFormatted = function () {
    ...
    for (let i = 0; i < sortedRaids.length; i++) {
        ...
        if (raid.state === RaidManager.RaidStateEnum.egg) {
            raidListMarkupRaidUpcoming.push(`[${tierString}] ${raid.raidLocation.name} @ ${formatDateAmPm(raid.hatchTime)}\n`);
        }
        else if (raid.state === RaidManager.RaidStateEnum.hatched) {
            raidListMarkupRaidActive.push(`[${tierString} ${raid.pokemon.name}] ${raid.raidLocation.name} ends @ ${formatDateAmPm(raid.expiryTime)}\n`);
        }
    }
