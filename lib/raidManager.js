...
RaidManager.prototype.getRaidsToSave = function () {
    let raids = [];
...
RaidManager.prototype.restoreRaidsFrom = function (allSaved) {
    let self = this;
    allSaved.forEach((saved) => {
        if (saved.expiry > Date.now()) {
            ...
            if (gym !== undefined) {
                ...
                console.log(`Restored: ${RaidManager.getFormattedRaidDescription(raid).description}`);
            }
        }
        else {
            console.log(`Expired: ${saved.gym}`);
        }
    });
};
...
RaidManager.prototype.tryRestoreState = function () {
    if (this.bosses && this.gyms && this.restoreFrom && fs.existsSync(this.restoreFrom)) {
        try {
            ...
            console.log(`Attempting to restore from ${restoreFrom}`);
            this.reportingSuspended = true;
            this.restoreRaidsFrom(JSON.parse(fs.readFileSync(restoreFrom, "utf8")));
            this.reportingSuspended = false;
            this.trySaveState();
        }
        catch (e) {
            console.log(`Error restoring raid state: ${e}`);
            this.reportingSuspended = false;
        }
    }
};
...
RaidManager.prototype.trySaveState = function () {
    if (this.autosaveFile && this.raids) {
        try {
            let stateToSave = JSON.stringify(this.getRaidsToSave());
            fs.writeFileSync(this.autosaveFile, stateToSave, { encoding: "utf8", flag: "w" });
            console.log(`Saved state to ${this.autosaveFile}.`);
        }
        catch (e) {
            console.log(`Error saving raid state to ${this.autosaveFile}: ${e}`);
        }
    }
};
