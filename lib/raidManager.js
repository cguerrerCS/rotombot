...
RaidManager.prototype.listDirectionsFormatted = function (filter) {
    let directions = "";
    this.list(filter).forEach((raid) => {
        directions += RaidManager.formatRaidDescription("GYM_NAME (FRIENDLY_NAME):\n -> _<MAP_LINK>_\n", raid);
    });
    return directions;
};

RaidManager.prototype.listRsvpsFormatted = function (filter) {
    let rsvps = "";
    this.list(filter).forEach((raid) => {
        let raiders = RaidManager.formatRaidDescription(" -> RAIDER_DETAILS", raid);
        if (raiders.length > 0) {
            rsvps += `${RaidManager.formatRaidDescription("GYM_NAME (FRIENDLY_NAME):\n", raid)}${raiders}`;
        }
    });
    return rsvps;
}

RaidManager.formatRaidDescription = function (description, raid) {
    const parts = [
        ...
        { placeholder: "RAIDER_DETAILS", replacement: (r) => Rsvp.classifyRaiders(r.raiders).details },
    ];
...
RaidManager.getFormattedRaidDescription = function (raid, eggFormat, bossFormat) {
    ...
    if (raid.state === RaidManager.RaidStateEnum.egg) {
        return {
            active: false,
            description: RaidManager.formatRaidDescription(eggFormat, raid),
        };
    }
    else if (raid.state === RaidManager.RaidStateEnum.hatched) {
        return {
            active: true,
            description: RaidManager.formatRaidDescription(bossFormat, raid),
        };
    }
