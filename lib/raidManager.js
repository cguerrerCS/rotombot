...
RaidManager.getAllRaiders = function (raid) {
    let raiders = { inbound: [], present: [], raiding: [], done: [] };
    for (let name in raid.raiders) {
        if (raid.raiders.hasOwnProperty(name)) {
            let raider = raid.raiders[name];
            if (raider.endTime) {
                raiders.done.push(raider);
            }
            else if (raider.startTime) {
                raiders.raiding.push(raider);
            }
            else if (raider.arrivalTime) {
                raiders.present.push(raider);
            }
            else {
                raiders.inbound.push(raider);
            }
        }
    }

    let numPending = raiders.present.length + raiders.inbound.length;
    raiders.summary = ((numPending < 1) ? "" : ((raiders.inbound.length < 1) ? `(${raiders.present.length})` : `(${raiders.present.length}/${numPending})`));
...
function formatRaidDescription(description, raid) {
    const parts = [
        ...
        { placeholder: "RAIDER_SUMMARY", replacement: (r) => RaidManager.getAllRaiders(r).summary },
    ];
