...
function validateBossObject(boss) {
    const required = ["name", "id", "tier", "image", "status"];
...
RaidManager.prototype.initDataAsync = function () {
    this.initGymDataAsync(fs.createReadStream("data/Gyms.csv", "utf8"));
    this.initBossDataAsync(fs.createReadStream("data/Bosses.csv", "utf8"));
};

RaidManager.prototype.initBossDataAsync = function (stream) {
    Utils.processCsvAsync(stream,
        (row) => {
            return {
                name: row[0],
                id: row[1],
...
RaidManager.prototype.findDefaultGymForRaider = function (raiderName) {
    ...
    this.forEachRaid((r) => gyms.push(r.gym), (r) => (r.raiders[raiderName] && (!r.raiders[raiderName].endTime) ? true : false));
