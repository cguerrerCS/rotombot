...
RaidManager.prototype.listFormatted = function () {
    ...
    for (let i = 0; i < sortedRaids.length; i++) {
        ...
        if (raid.state === RaidManager.RaidStateEnum.egg) {
            raidListMarkupRaidUpcoming.push(`[${tierString}] ${raid.gym.name} @ ${formatDateAmPm(raid.hatchTime)}\n`);
        }
        else if (raid.state === RaidManager.RaidStateEnum.hatched) {
            raidListMarkupRaidActive.push(`[${tierString} ${raid.pokemon.name}] ${raid.gym.name} ends @ ${formatDateAmPm(raid.expiryTime)}\n`);
        }
    }
