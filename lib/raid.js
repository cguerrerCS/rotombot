...
const activeRaidFormatOptions = {
    formatsByDestination: {
        text: "[TIER BOSS_NAME] GYM_NAME ends @ EXPIRY_TIME RAIDER_SUMMARY",
        markdown: "[TIER BOSS_NAME] GYM_NAME *ends @ EXPIRY_TIME* RAIDER_SUMMARY",
        embed: "[[TIER BOSS_NAME](GUIDE_LINK)] [GYM_NAME](MAP_LINK) *ends @ EXPIRY_TIME* RAIDER_SUMMARY",
    },
    parts: formatParts,
};
...
const upcomingRaidFormatOptions = {
    formatsByDestination: {
        text: "[TIER] GYM_NAME @ HATCH_TIME RAIDER_SUMMARY",
        markdown: "[TIER] GYM_NAME *@ HATCH_TIME* RAIDER_SUMMARY",
        embed: "[TIER] [GYM_NAME](MAP_LINK) *@ HATCH_TIME* RAIDER_SUMMARY",
    },
    parts: formatParts,
};
...
Raid.raidToDiscordMessage = function (raid) {
    ...
    let thumbnail = `http://www.didjaredo.com/pogo/images/32x32/${raid.pokemon ? "regice" : "t5"}.png`;
...
Raid.getFormattedRaidListOutputAsString = function (raids, headings, options) {
    ...
    if (output.length < 1) {
        if (headings.noRaidsMessage) {
            ...
            return `No raids to report (${headings.description}).`;
        }
...
Raid.raidListToDiscordMessage = function (raids, headings, options) {
    ...
    if (!message.embed.title) {
        if (headings.noRaidsMessage) {
            ...
            message.embed.title = `No raids to report (${headings.description}).`;
        }
        else {
            message.embed.title = "No raids to report.";
        }
    }
