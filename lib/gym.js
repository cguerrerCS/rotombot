...
Gym.parseV1CsvRow = function (csv) {
    ...
    return {
        ...
        aliases: aliases,
        latitude: (typeof csv[4] === "number" ? csv[4] : parseFloat(csv[4])),
...
Gym.prototype.toCsvRow = function () {
    let uid = this.uid;
    let zones = (this.zones.length === 1) ? this.zones[0] : this.zones.join("|");
    let city = this.city;
    let official = this.officialName;
    let friendly = this.friendlyName;
    if (this.aliases && (this.aliases.length > 0)) {
        friendly = `friendly|${this.aliases.join("|")}`;
    }
    let long = this.longitude;
    let lat = this.latitude;
    let ex = (this.isExEligible) ? "ExEligible" : "NonEx";
    return `${uid},${zones},${city},${official},${friendly},${long},${lat},${ex}`;
};
